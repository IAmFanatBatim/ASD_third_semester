...
//Возвращает значение "истина" с вероятностью GENERATION_PERCENT процентов
bool shouldGenerateTask() {
    return rand() % 100 < GENERATION_PERCENT;
}
...
// 2) случайным временем выполнения (варьируется от 2 до 10 тактов),
// 3) случайным приоритетом (значения 0, 1, 2 равновероятны)
...
//Выводит на экран задачу T в формате (имя;время)
